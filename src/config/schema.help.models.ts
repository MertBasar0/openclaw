...
export const MODEL_FIELD_HELP: Record<string, string> = {
  ...
  "agents.defaults.experimental.decisionAssistance":
    "Global opt-in for experimental Decision assistance (default: false; requires explicit true). With an effective decisionModel inherited from global defaults or selected per agent, the built-in OpenClaw runtime can omit optional tools on eligible conversational turns while preserving host-required tools and permissions. Other harnesses keep their normal tools. The current request text is sent to the selected Decision provider; hosted providers may charge. An empty per-agent decisionModel disables assistance. This setting does not gate explicit decision_evaluate calls, select providers, or grant actions.",
