...
<AccordionGroup>
  ...
  <Accordion title="1. Config normalization">
    ...
    A tool policy scope that sets both `allow` and `alsoAllow` fails validation. `doctor --fix` merges `alsoAllow` into `allow` for the affected non-sandbox scope, at the root `tools` policy, per-agent and per-provider policies, and channel or gateway tool policies, and reports each merge. Sandbox tool policy lists remain untouched because `allow` and `alsoAllow` inherit independently; conflicting sandbox lists still require manual repair. Plugin-owned `plugins.entries.*.config` is left to the owning plugin's doctor contract. Gateway startup applies the same repair for these non-sandbox scopes, so an otherwise valid config carrying only a supported conflict no longer blocks boot.
